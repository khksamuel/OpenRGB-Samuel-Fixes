...
struct SoftwareRainbowState
{
    ...
    std::thread       thread;
    int               speed = ENE_SPEED_NORMAL;
    int               direction = ENE_DIRECTION_FORWARD;
};
...
void StartSoftwareRainbow(RGBController_ENESMBus* controller, int speed, int direction)
{
    ...
    if(!state->thread.joinable())
    {
        state->thread = std::thread([controller, state]
        {
            ...
            {
                /*
                 * G.SKILL's factory Rainbow is generated by the ENE device
                 * itself.  Re-arm it periodically so separate DIMM timers do
                 * not slowly drift, while leaving the native animation and
...
void RGBController_ENESMBus::DeviceUpdateMode()
{
    ...
    {
        const int speed = modes[active_mode].speed;
        const int direction = modes[active_mode].direction == MODE_DIRECTION_RIGHT
            ? ENE_DIRECTION_REVERSE : ENE_DIRECTION_FORWARD;

        controller->SetMode(ENE_MODE_RAINBOW, speed, direction);
        controller->SetDirect(false);
        StartSoftwareRainbow(this, speed, direction);
        return;
    }
...
void RGBController_ENESMBus::ReapplyNativeRainbow(int speed, int direction)
{
    controller->SetMode(ENE_MODE_RAINBOW, speed, direction);
    controller->SetDirect(false);
}
