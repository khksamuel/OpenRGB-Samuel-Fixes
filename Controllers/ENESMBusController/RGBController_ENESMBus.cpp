...
void RGBController_ENESMBus::DeviceUpdateLEDs()
{
    ...
    {
        /*
         * The Trident Z factory effect is a broad rainbow wave that travels
         * down the five-zone diffuser.  It does not squeeze a complete HSV
         * wheel into every DIMM.  Use G.SKILL's documented named colors as
         * equally timed palette stops and one stop of spacing per LED.
         */
        ...
        const double cycle_ms = 5000.0 + (static_cast<double>(rainbow_mode.speed) * 1500.0);
        const double palette_phase = GetSharedRainbowElapsedMilliseconds() / cycle_ms * palette.size();
        const bool reverse = rainbow_mode.direction == MODE_DIRECTION_RIGHT;

        for(std::size_t led_idx = 0; led_idx < leds.size(); led_idx++)
        {
            double position = reverse
                ? palette_phase + static_cast<double>(led_idx)
                : palette_phase - static_cast<double>(led_idx);

            position = std::fmod(position, static_cast<double>(palette.size()));
            if(position < 0.0)
            {
            ...
            const std::size_t second = (first + 1) % palette.size();
            const double blend = position - static_cast<double>(first);

            const unsigned char red = static_cast<unsigned char>(
                palette[first][0] + (palette[second][0] - palette[first][0]) * blend);
            const unsigned char green = static_cast<unsigned char>(
                palette[first][1] + (palette[second][1] - palette[first][1]) * blend);
            const unsigned char blue = static_cast<unsigned char>(
                palette[first][2] + (palette[second][2] - palette[first][2]) * blend);
